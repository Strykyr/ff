...
from models import Attention, Informer, Autoformer, Transformer, DLinear, Linear, NLinear,Resnet_LSTM,Resnet,LSTM,Attention_LSTM,Attention,BILSTM
...
intervention_dict = {'11': 0, '12': 1, '13': 2, 'ts1': 3}
input_id = torch.LongTensor([intervention_dict['11'],intervention_dict['12'],intervention_dict['13']])  # 批量输入3个干预编号
...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        val_loder = my_data("val",self.test_data)
    ...
    def test(self, setting, test=0):
        ...
        if test:
           ...
           #self.model.load_state_dict(torch.load(os.path.join(self.folder_path + self.args.model + '/checkpoints/', 'checkpoint.pth')))
            self.model.load_state_dict(torch.load(os.path.join(self.folder_path + self.args.model + '/checkpoints/', 'checkpoint.pth')),False)
